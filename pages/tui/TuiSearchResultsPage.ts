...
export class TuiSearchResultsPage extends BasePage<{}> {
  ...
  async waitForResults(): Promise<void> {
    ...
    const headerSelectors = ['[data-testid="results-header"]', 'h1', 'h2', '.SearchResults__header', 'div'];
    let header: ReturnType<Page['locator']> | null = null;
    for (const sel of headerSelectors) {
      const cand = this.page.locator(sel).filter({ hasText: /hotel|vakantie|result|gevonden|results/i }).first();
