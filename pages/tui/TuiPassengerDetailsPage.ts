...
const selectors = {
  pageHeading: 'h1',
  submitButton: 'button:has-text("Continue"), button:has-text("Ga verder"), button:has-text("Doorgaan"), button[type="submit"], .ProgressbarNavigation__summaryButton button',
} as const;
...
export class TuiPassengerDetailsPage extends BasePage<typeof selectors> {
  ...
  async pageLoaded(): Promise<this> {
    ...
    const heading = this.page.locator('h1').filter({ hasText: /Persoonsgegevens|Passenger details/i });
  ...
  private getErrorMessageLocator(passengerKey: string, fieldName: string) {
    ...
    const visibleError = `.inputs__errorMessage:not(.inputs__hidden), [role="alert"]:not(.inputs__hidden)`;
